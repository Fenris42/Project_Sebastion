...
public class Player_Attack : MonoBehaviour
{
    ...
    private void SpawnArrow()
    {
        ...
        float x = (float)(transform.position.x - 0.5); //offset arrow to be not centered on player
        float y = (float)(transform.position.y + 1); //offset arrow to be towards hands

        //spawn fireball
        Instantiate(arrowPrefab, new Vector3(x, y), transform.rotation);
    }
